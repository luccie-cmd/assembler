...
> struct filter_impl<cond, list<processed...>, next, rest...> {
    ...
    >::type;
        ...
        template <typename t>
        concept is_vector_v = test_vector<t>::value;
        template <typename t>
        using remove_vector_t = typename test_vector<t>::type;
        ...
        template <typename opt>
        concept is_positional_v = is_positional<opt>::value;
        ...
        template <auto cb>
        struct make_lambda_s;

        template <auto cb>
            requires std::is_invocable_v<decltype(cb)>
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *, std::string_view)
                                    { cb(); });
        ...
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *data, std::string_view)
                                    { cb(data); });
            using type = callback_noarg_type;
        };
        ...
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *, std::string_view, std::string_view arg)
                                    { cb(arg); });
        ...
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *data, std::string_view, std::string_view arg)
                                    { cb(data, arg); });
            using type = callback_arg_type;
        };
        ...
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *, std::string_view name, std::string_view arg)
                                    { cb(name, arg); });
            using type = callback_arg_type;
        };
        ...
        struct make_lambda_s<cb>
        {
            using lambda = decltype([](void *data, std::string_view name, std::string_view arg)
                                    { cb(data, name, arg); });
            using type = callback_arg_type;
        };
...
> or is_vector_v<type> or requires { type::is_values; } or requires { type::is_file_data; };
        ...
        struct opt_impl
        {
                ...
                const single_element_type &val)
            {
                if constexpr (is_values)
                    return declared_type_base::is_valid_option_value(val);
                else
                    return true;
            }
            ...
            {
                if constexpr (is_values)
                    return declared_type_base::print_values(out);
                else
                    return 0;
            }
        ...
        static file_data_type map_file(
            ...
            auto error_handler = [](std::string &&msg)
            { std::cerr << msg << "\n"; std::exit(1); })
        {
            ...
            if constexpr (requires { ret.assign(pointer, sz); })
                ret.assign(pointer, sz);
            else if constexpr (requires { ret.assign(pointer, pointer + sz); })
                ret.assign(pointer, pointer + sz);
            else
                CLOPTS_ERR("file_data_type::contents_type must have a callable assign member that takes a pointer and a size_t (or a begin and end iterator) as arguments.");
            ::munmap(mem, sz);
        ...
        static consteval auto type_name() -> static_string<25>
        {
            ...
            if constexpr (detail::is<t, std::string>)
                buffer.append("string");
            else if constexpr (detail::is<t, bool>)
                buffer.append("bool");
            else if constexpr (detail::is<t, std::int64_t, double>)
                buffer.append("number");
            else if constexpr (requires { t::is_file_data; })
                buffer.append("file");
            else if constexpr (detail::is_callback<t>)
                buffer.append("arg");
        ...
        class clopts_impl<list<opts...>, list<special...>>
        {
            ...
            static constexpr size_t optindex_impl()
            {
                if constexpr (index >= sizeof...(opts))
                    return index;
                else if constexpr (CLOPTS_STRCMP(opt_names[index], option.arr) == 0)
                    return index;
                else
                    return optindex_impl<index + 1, option>();
            }
            ...
            {
                auto is_mul = []<typename opt>()
                { return requires { opt::is_multiple; }; };
                return (... + (is_mul.template operator()<opts>() and detail::is_positional_v<opts>));
            }

            template <typename type, static_string... references>
            static consteval bool validate_references_impl(ref<type, references...>)
            { // clang-format off
    ...
    >>; // clang-format on
            ...
            struct compute_ref_storage_type<declared_type, ref<declared_type_base, args...>>
            { // clang-format off
        ...
        using type = std::conditional_t<is_vector_v<declared_type>, std::vector<tuple>, tuple>;
            ...
            struct storage_type
            {
                using type = std::conditional_t<
                    ...
                    std::type_identity<typename opt::canonical_type>>::type;
            };
    ...
    >>; // clang-format on
        ...
        public:
            ...
            class optvals_type
            {
                ...
                constexpr auto get_impl() -> get_return_type<opt_by_name<s>>
                {
                    ...
                    if constexpr (std::is_same_v<canonical, bool>)
                        return opts_found[optindex<s>()];

                    // We always return a span to multiple<> options because the user can just check if it’s empty.
                    else if constexpr (detail::is_vector_v<canonical>)
                        return std::get<optindex<s>()>(optvals);

                    // Function options don’t have a value.
                    else if constexpr (detail::is_callback<canonical>)
                        CLOPTS_ERR("Cannot call get<>() on an option with function type.");

                    // Otherwise, return nullptr if the option wasn’t found, and a pointer to the value otherwise.
                    else
                        return not opts_found[optindex<s>()] ? nullptr : std::addressof(std::get<optindex<s>()>(optvals));
                }
        ...
        private:
            ...
            auto parse_number(std::string_view s, auto parse_func) -> number_type
            {
                ...
                errno = 0;
                char *pos{};
                if constexpr (requires { parse_func(s.data(), &pos, 10); })
                    i = number_type(parse_func(s.data(), &pos, 10));
                else
                    i = number_type(parse_func(s.data(), &pos));
                if (errno != 0 or *pos)
                    handle_error(s, " does not appear to be a valid ", name.sv());
                return i;
            }
            ...
            [[nodiscard]] constexpr auto ref_to_storage() -> decltype(std::get<optindex<s>()>(optvals.optvals)) &
            {
                ...
                if constexpr (std::is_same_v<value_type, bool>)
                    CLOPTS_ERR("Cannot call ref() on an option<bool>");

                // Function options don’t have a value.
                else if constexpr (detail::is_callback<value_type>)
                    CLOPTS_ERR("Cannot call ref<>() on an option with function type.");
        ...
        private:
            ...
            void add_referenced_option(auto &tuple)
            {
                ...
                if (found<name>())
                {
                    ...
                    if constexpr (opt::is_flag)
                        storage = true;
                    else if constexpr (is_vector_v<storage_type_t<opt>>)
                        storage = ref_to_storage<name>();
                    else
                        storage = std::make_optional(*optvals.template get<name>());
                }
            }
            ...
            void dispatch_option_with_arg(std::string_view opt_str, std::string_view opt_val)
            {
                ...
                if constexpr (detail::is_callback<canonical>)
                {
                    if constexpr (detail::is<canonical, callback_noarg_type>)
                        ...
                        opt::callback(user_data, opt_str, opt_val);
                }
            ...
            bool handle_regular_impl(std::string_view opt_str)
            {
                ...
                if (not opt_str.starts_with(opt::name.sv()))
                    return false;
                ...
                {
                    return handle_opt_with_arg<opt, is_multiple>(opt_str);
                }
            }
            ...
            bool handle_regular(std::string_view opt_str)
            {
                const auto handle = [this]<typename opt>(std::string_view str)
                {
                    ...
                    if constexpr (detail::is_positional_v<opt>)
                        ...
                        return this->handle_regular_impl<opt>(str);
                };
            ...
            bool handle_positional(std::string_view opt_str)
            {
                const auto handle = [this]<typename opt>(std::string_view str)
                {
                    // `this->` is to silence a warning.
                    if constexpr (detail::is_positional_v<opt>)
                        return this->handle_positional_impl<opt>(str);
                    else
                        return false;
                };
            ...
            auto make_arg(std::string_view opt_val) -> typename opt::single_element_type
            {
                ...
                if constexpr (not detail::has_argument<element>)
                    CLOPTS_ERR("This option type does not take an argument");

                // Strings do not require parsing.
                else if constexpr (std::is_same_v<element, std::string>)
                    return std::string{opt_val};

                // If it’s a file, read its contents.
                else if constexpr (requires { element::is_file_data; })
                    return detail::map_file<element>(opt_val, error_handler);

                // Parse an integer or double.
                else if constexpr (std::is_same_v<element, integer>)
                    return parse_number<integer, "integer">(opt_val, std::strtoull);
                else if constexpr (std::is_same_v<element, double>)
                    return parse_number<double, "floating-point number">(opt_val, std::strtod);

                // Should never get here.
                else
                    CLOPTS_ERR("Unreachable");
            }
            ...
            {
                if constexpr (requires { opt::is_stop_parsing; })
                    return opt_str == opt::name.sv();
                return false;
            }
        ...
        public:
            ...
            static auto parse(
                ...
                void *user_data = nullptr) -> optvals_type
            {
                // Initialise state.
                clopts_impl self;
                if (error_handler)
                    ...
                    self.error_handler = [&](auto &&e)
                    { return self.default_error_handler(std::forward<decltype(e)>(e)); };
                self.argc = argc;
